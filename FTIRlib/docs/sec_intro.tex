...
Este informe técnico documenta el diseño teórico, los algoritmos matemáticos y los resultados experimentales de la versión \texttt{0.0.4} de la biblioteca unificada \texttt{ftir\_library} (\texttt{ftirlib}). Esta biblioteca integra bajo un mismo núcleo computacional robusto desarrollado en Python puro (NumPy y SciPy) el procesamiento y análisis automatizado de dos de las técnicas espectroscópicas vibracionales más relevantes en la actualidad: la espectroscopía infrarroja por transformada de Fourier (FTIR) y la espectroscopía Raman amplificada por superficies (SERS).
...
El documento describe detalladamente la validación matemática y la flexibilidad operacional de la biblioteca a través de cinco casos de aplicación de complejidad creciente:
\begin{itemize}
    ...
    \item \textbf{Caso de Aplicación IV}: Comparación de calidad de proveedores de grafeno mediante Análisis de Componentes Principales (PCA) basado en SVD y clasificación con Kernel Ridge, validando la separabilidad de los lotes de rGO con un 100\% de exactitud.
    \item \textbf{Caso de Aplicación V}: Automatización de muestreo de espectros infrarrojos clásicos de la base de datos NIST Chemistry WebBook, demostrando el lector nativo de archivos JCAMP-DX (\path{.jdx}) y el uso de la API simplificada con constantes \texttt{RAW} y \texttt{NONE}.
\end{itemize}
\newpage